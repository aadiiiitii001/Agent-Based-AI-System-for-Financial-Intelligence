...
class RiskAgent:
    ...
    def get_us_data(self, symbol: str) -> dict:
        """Fetch US stock data from AlphaVantage."""
        try:
            if symbol == "BTC":
                url = f"{self.alpha_url}?function=CURRENCY_EXCHANGE_RATE&from_currency=BTC&to_currency=USD&apikey={self.api_key}"
    ...
    def run(self, query: str) -> dict:
        ...
        if not data:
            return {"status": "skipped", "reason": f"Could not fetch real data for {symbol}"}
        ...
        return {
            ...
            "volatility": "High" if risk_score > 0.65 else "Medium" if risk_score > 0.35 else "Low",
            "assessment": self.interpret_risk(risk_score),
            "data_source": "Yahoo Finance (Real-time)" if market == "IN" else "AlphaVantage (Real-time)"
        }
